...
################################################################################
# Generated by:9.1
# variation name : DDR2_IF_phy
# family : Cyclone III
# speed_grade : 8
# local_if_drate : HALF
# pll_ref_clk_mhz : 125.0
# mem_if_clk_mhz : 166.667
# mem_if_preset : Micron MT47H16M16BG-3
...
} elseif {$::TimeQuestInfo(family) == "Cyclone III" || $::TimeQuestInfo(family) == "Cyclone III LS" || $::TimeQuestInfo(family) == "Cyclone IV GX"} {
...
################################################################################
set pin_file_name "DDR2_IF_phy_ddr_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name DDR2_IF_phy_use_high_performance_timing} {
	upvar 1 $t_name t
...
# Create the PLL input clock and derive clocks on the PLL outputs
	...
	if {$ck_pll_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "DDR2_IF_phy_ddr_timing.sdc: $msg"
		}
		post_message -type warning "DDR2_IF_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
	} else {
		...
		if {$pll_ref_clk_id != -1} {
			...
			post_message -type info "DDR2_IF_phy_ddr_timing.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
			# Attempt to recover
...
# Find the TimeQuest name for the resync clock. If it is not found, create one.
	...
	if {$resync_pll_ref_clk_id != -1} {
		...
		post_message -type warning "DDR2_IF_phy_ddr_timing.sdc: Failed to find PLL input clock pin driving $resync_clock"
	}
...
################################################################################
	...
	if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
		...
		post_message -type warning "DDR2_IF_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
	} else {
...
################################################################################
	...
	if {$ac_pll_clock_id == -1} {
		...
		post_message -type warning "DDR2_IF_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
	} else {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t $DDR2_IF_phy_use_high_performance_timing
